...
function Y(e, t, u, S = (g) => g) {
  return e * S(0.5 - t * (0.5 - u));
}
function p(e, t) {
...
function V(e, t, u) {
  let S = Math.sin(u), g = Math.cos(u), y = e[0] - t[0], o = e[1] - t[1], x = y * g - o * S, L = y * S + o * g;
  return [x + t[0], L + t[1]];
}
function N(e, t, u) {
...
var { min: _, PI: de } = Math;
var ie = 0.275;
var j = de + 1e-4;
function pe(e, t = {}) {
  let { size: u = 16, smoothing: S = 0.5, thinning: g = 0.5, simulatePressure: y = true, easing: o = (n) => n, start: x = {}, end: L = {}, last: q = false } = t, { cap: k = true, taper: P = 0, easing: C = (n) => n * (2 - n) } = x, { cap: b = true, taper: a = 0, easing: T = (n) => --n * n * n + 1 } = L;
  if (e.length === 0 || u <= 0)
    return [];
  let U = e[e.length - 1].runningLength, B = Math.pow(u * S, 2), O = [], v = [], X = e.slice(0, 10).reduce((n, i) => {
    let r = i.pressure;
    if (y) {
      let s = _(1, i.distance / u), l = _(1, 1 - s);
      r = _(1, n + (l - n) * (s * ie));
    }
    return (n + r) / 2;
  }, e[0].pressure), c = Y(u, g, e[e.length - 1].pressure, o), J, A = e[0].vector, z = e[0].point, F = z, E = z, M = F;
  for (let n = 0; n < e.length - 1; n++) {
    let { pressure: i } = e[n], { point: r, vector: s, distance: l, runningLength: R } = e[n];
    if (U - R < 3)
      continue;
    if (g) {
      if (y) {
        let D2 = _(1, l / u), W = _(1, 1 - D2);
        i = _(1, X + (W - X) * (D2 * ie));
      }
      c = Y(u, g, i, o);
    } else
      c = u / 2;
    J === void 0 && (J = c);
    let ae = R < P ? C(R / P) : 1, le = U - R < a ? T((U - R) / a) : 1;
    c = Math.max(0.01, c * Math.min(ae, le));
    let ee = e[n + 1].vector, te = re(s, ee);
    if (te < 0) {
      let D2 = f(I(A), c);
      for (let W = 1 / 13, H = 0; H <= 1; H += W)
        E = V(m(r, D2), r, j * H), O.push(E), M = V(p(r, D2), r, j * -H), v.push(M);
      z = E, F = M;
      continue;
    }
  ...
  let d = e[0].point.slice(0, 2), h = e.length > 1 ? e[e.length - 1].point.slice(0, 2) : p(e[0].point, [1, 1]), Q = O.length <= 1 || v.length <= 1, w = [], G = [];
  if (Q) {
    if (!(P || a) || q) {
      let n = $(d, K(I(m(d, h))), -(J || c)), i = [];
      for (let r = 1 / 13, s = r; s <= 1; s += r)
        i.push(V(n, d, j * 2 * s));
      return i;
    }
  } else {
    if (P || a && Q)
      ...
      let r = m(O[0], v[0]), s = f(r, 0.5), l = f(r, 0.51);
      w.push(m(d, s), m(d, l), p(d, l), p(d, s));
    }
    let n = ue(O[O.length - 1], v[v.length - 1]), i = I(K(m(h, n)));
    if (a || P && Q)
      G.push(h, p(h, [1, 0]));
    else if (b) {
      let r = $(h, i, c);
      ...
      G.push(p(h, f(i, c)), p(h, f(i, c * 0.99)), m(h, f(i, c * 0.99)), m(h, f(i, c)));
  }
  return O.concat(G, v.reverse(), w);
}
function me(e, t = {}) {
  var C;
  let { streamline: u = 0.5, size: S = 16, last: g = false } = t;
  if (e.length === 0)
    return [];
  let y = 0.15 + (1 - u) * 0.85, o = Array.isArray(e[0]) ? e : e.map(({ x: b, y: a, pressure: T = 0.5 }) => [b, a, T]);
  o.length === 1 && (o = [...o, [...p(o[0], [1, 1]), ...o[0].slice(2)]]);
  let x = [{ point: [o[0][0], o[0][1]], pressure: o[0][2] || 0.25, vector: [1, 1], distance: 0, runningLength: 0 }], L = false, q = 0, k = x[0], P = o.length - 1;
  for (let b = 1; b < o.length; b++) {
    ...
    k = { point: a, pressure: o[b][2] || 0.5, vector: K(m(k.point, a)), distance: T, runningLength: q }, x.push(k);
  }
...
var StylusModel = class extends BaseModel {
  ...
  getSvgData(points) {
    const stroke = qe(points, __spreadValues({
