...
function resolveFonts(fonts = {}) {
  ...
  let sans = (0, import_utils2.toArray)(fonts.sans).flatMap((i) => i.split(/,\s*/g)).map((i) => i.trim());
  let serif = (0, import_utils2.toArray)(fonts.serif).flatMap((i) => i.split(/,\s*/g)).map((i) => i.trim());
  let mono = (0, import_utils2.toArray)(fonts.mono).flatMap((i) => i.split(/,\s*/g)).map((i) => i.trim());
  const weights = (0, import_utils2.toArray)(fonts.weights || "200,400,600").flatMap((i) => i.toString().split(/,\s*/g)).map((i) => i.trim());
  const custom = (0, import_utils2.toArray)(fonts.custom).flatMap((i) => i.split(/,\s*/g)).map((i) => i.trim());
  const local = (0, import_utils2.toArray)(fonts.local).flatMap((i) => i.split(/,\s*/g)).map((i) => i.trim());
  const webfonts = fonts.webfonts ? fonts.webfonts : fallbacks ? (0, import_utils2.uniq)([...sans, ...serif, ...mono, ...custom]) : [];
...
function matter(code) {
  ...
  const content = code.replace(/^---.*\r?\n([\s\S]*?)---/, (_, d) => {
    ...
    if (!(0, import_utils4.isObject)(data))
      data = {};
    return "";
  });
  return { data, content };
...
function mergeFeatureFlags(a, b) {
  return (0, import_utils4.objectMap)(a, (k, v) => [k, v || b[k]]);
}
function scanMonacoModules(md) {
  ...
  md.replace(/^```(\w+?)\s*{monaco([\w:,-]*)}[\s\n]*([\s\S]+?)^```/mg, (full, lang = "ts", options, code) => {
    ...
    if (lang === "ts" || lang === "typescript") {
      Array.from(code.matchAll(/\s+from\s+(["'])([\/\w@-]+)\1/g)).map((i) => i[2]).filter(import_utils4.isTruthy).map((i) => typeModules.add(i));
    }
    return "";
  });
