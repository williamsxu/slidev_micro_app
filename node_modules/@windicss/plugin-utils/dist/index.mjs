...
async function resolveOptions(options = {}, utilsOptions = {}, loadConfigFile = false) {
  ...
  let { config, filepath: configFilePath } = loadConfigFile ? loadConfiguration(__spreadProps(__spreadValues({
...
function createUtils(userOptions = {}, utilsOptions = {
  ...
  const scan = createSingletonPromise(async () => {
    ...
    const contents = await Promise.all(files.filter((id) => isDetectTarget(id)).map(async (id) => [await fs.promises.readFile(id, "utf-8"), id]));
